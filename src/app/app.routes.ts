...
export const routes: Routes = [
    {
        path: '',
        pathMatch: 'full',
        redirectTo: () => localStorage.getItem(TOKEN_KEY) ? '/home' : '/login'
    },
    ...
    {path: 'browse', loadComponent: () => import('./browsetools/browsetools.component').then(m => m.BrowseToolsComponent), title: "Browse tools", canActivate: [authGuard] },

    // Login & register
    {path: 'register', loadComponent: () => import('./user/register/register.component').then(m => m.RegisterComponent), title: "Register"},
    {path: 'login', loadComponent: () => import('./user/login/login.component').then(m => m.LoginComponent), title: "Login", canActivate: [rootRedirectGuard]},
];
